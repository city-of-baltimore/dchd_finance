...
for (cost_center in cost_center_list) {
  ...
  salary_proj <- sum(as.numeric(cost_center_data$`Revised Budget`[str_detect(cost_center_data$`Spend Category`, salary_pattern)]), na.rm = TRUE)
  non_salary_proj <- sum(as.numeric(cost_center_data$`Revised Budget`[!str_detect(cost_center_data$`Spend Category`, salary_pattern)]), na.rm = TRUE)
  
  # store results in a dataframe
  general_fund_results[[as.character(cost_center)]] <- data.frame(
    `Cost Center` = cost_center,
    `Salaries/OPC Projections` = salary_proj,
    `Other EXP Projections` = non_salary_proj,
    `Total Projections` = salary_proj + non_salary_proj
  )
}
...
for (cost_center in cost_center_list) {
  ...
  # compute salary and non-salary projections
  salary_proj <- sum(as.numeric(cost_center_data$`GL YTD EXP`[str_detect(cost_center_data$`Spend Category`, salary_pattern)]), na.rm = TRUE)
  non_salary_proj <- sum(as.numeric(cost_center_data$`GL YTD EXP`[!str_detect(cost_center_data$`Spend Category`, salary_pattern)]), na.rm = TRUE)
