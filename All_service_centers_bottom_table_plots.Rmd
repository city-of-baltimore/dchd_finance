...
*First calculations include the Total Budget*
...
for (cost_center in gf_cost_center_list) {
  ...
  gf_salary_proj <- sum(as.numeric(cost_center_data$`Total Budget`[str_detect(cost_center_data$`Spend Category`, salary_pattern)]), na.rm = TRUE)
  gf_non_salary_proj <- sum(as.numeric(cost_center_data$`Total Budget`[!str_detect(cost_center_data$`Spend Category`, salary_pattern)]), na.rm = TRUE)
...
for (cost_center in sf_cost_center_list) {
  ...
  # compute salary and non-salary projections
  sf_salary_proj <- sum(as.numeric(cost_center_data$`Total Budget`[str_detect(cost_center_data$`Spend Category`, salary_pattern)]), na.rm = TRUE)
  sf_non_salary_proj <- sum(as.numeric(cost_center_data$`Total Budget`[!str_detect(cost_center_data$`Spend Category`, salary_pattern)]), na.rm = TRUE)
...
# Step 2: sum the Total Budget from both of the salary and non salary rows because these contain the Total Budget totals
...
ggplot(gf_glytd_comparison_data, aes(x = Category, y = Value, fill = Category)) +
  ...
  # Annotate the Total Budget line with text
  annotate("text", x = 3.5, y = total_projections_gf,  # Place text at the intersection of the two segments
           label = paste("Total Budget", label_dollar()(total_projections_gf)), 
           color = "blue", vjust = -0.1, size = 7, fontface = "bold") + 
...
# save the plot
ggsave("all_gf_glytd_comparison_data.png",width = 28, height = 18, dpi = 300)  
```
...
ggplot(sg_glytd_comparison_data, aes(x = Category, y = Value, fill = Category)) +
  ...
  annotate("text", x = 3.5, y = total_projections_sg,  # Place text at the intersection of the two segments
           label = paste("Total Budget", label_dollar()(total_projections_sg)), 
           color = "blue", vjust = -0.1, size = 7, fontface = "bold") + 
  scale_fill_manual(values = c("Salaries/OPC Projections" = "steelblue", 
                               "Other EXP Projections" = "darkgreen",
                               "Total Projections" = "gray")) +  # Custom colors
...
# save the plot
ggsave("all_sg_glytd_comparison_data.png",width = 28, height = 18, dpi = 300)  
```
...
all_tpe_glytd_comparison_data <- data.frame(
  ...
  Fund = c("1001 General Grants Fund", "GL YTD EXP 1001 General Grants Fund",
           ...
           "Total Projections Total Budget", "Total Projections General Ledger YTD"),  
  Value = c(
...
# Step 2: sum the Total Budget Correctly
...
# Step 2: calculate the total Total Budget for the first rows defined in the dataset
total_projections_sg <- sum(as.numeric(sf_salary_proj), na.rm = TRUE) + 
                        sum(as.numeric(sf_non_salary_proj), na.rm = TRUE)
