---
title: "SRV0737 Administration - HCD Plots Deputy Marvin Blye"
...
*Note: This will make plots for `SRV0737 Administration - HCD` only and will be for Deputy Dr. Marvin Blye*
...
# Plots for Deputy Dr. Marvin Blye
...
*Service* 
  1.`SRV0737 Administration - HCD`
*Cost Center* 
  1.`CCA000310 Housing Budget and Accounting (HCD)`
  2.`CCA000311 Housing Communications (HCD)`
  3.`CCA000312 Housing Executive Direction (HCD)`
  4.`CCA000313 Housing Facilities Management (HCD)`
  5.`CCA000314 Housing Information Technology (HCD)`
  6.`CCA000315 Housing Human Resources (HCD)`
  7.`CCA000316 Housing Planning and Development (HCD)`
  8.`CCA001173 Performance and Analytics (HR)`
  9.`CCA001174 HCD Administration Unallocated Appropriation`
...
admin_mb_negative_spending <- budg_vs_actuals |>
  filter(
    Service == "SRV0737 Administration - HCD" &
    `Cost Center` %in% c("CCA000310 Housing Budget and Accounting (HCD)",
                         "CCA000311 Housing Communications (HCD)",
                         "CCA000312 Housing Executive Direction (HCD)",
                         "CCA000313 Housing Facilities Management (HCD)",
                         "CCA000314 Housing Information Technology (HCD)",
                         "CCA000315 Housing Human Resources (HCD)",
                         "CCA000316 Housing Planning and Development (HCD)",
                         "CCA001173 Performance and Analytics (HR)",
                         "CCA001174 HCD Administration Unallocated Appropriation") &
    `Total Spent` < 0
  )

# plotting data below for different perspectives
admin_mb_tot_neg_ccenter <- ggplot(admin_mb_negative_spending, aes(x = reorder(`Cost Center`, `Total Spent`), y = `Total Spent`)) +
  ...
  labs(
    ...
    subtitle = "Service: SRV0737 Administration - HCD",
    x = "Cost Center",
    y = "Total Spent (USD)",
...
# display the plot
print(admin_mb_tot_neg_ccenter)

# saving the plot
ggsave("admin_mb_tot_neg_ccenter_deputy_marvin.pdf", 
       width = 30, height = 20)
```
...
```{r spendcat}
# plotting data below for different perspectives
admin_mb_tot_neg_spcat <- ggplot(admin_mb_negative_spending, aes(x = reorder(`Spend Category`, `Total Spent`), y = `Total Spent`)) +
  geom_col(fill = "red", width = 0.5) +  # Adjust bar width for better aesthetics
  theme_minimal() +
  scale_y_reverse(labels = scales::dollar) +  # Reverse y-axis and format with dollar labels
  labs(
...
# display the plot
print(admin_mb_tot_neg_spcat)

# saving the plot
ggsave("admin_mb_tot_neg_spcat_deputy_marvin.pdf", 
       width = 30, height = 20)
```
...
admin_mb_actual_spent <- budg_vs_actuals |>
  filter(
    Service == "SRV0737 Administration - HCD" &
    `Cost Center` %in% c("CCA000310 Housing Budget and Accounting (HCD)",
                         "CCA000311 Housing Communications (HCD)",
...
admin_mb_actual_spent_long <- admin_mb_actual_spent |> 
  pivot_longer(
...
admin_mb_actual_spent_long <- admin_mb_actual_spent_long |> 
  filter(!is.na(Month))

# Now, create a sequence of months from the minimum to the current month
full_months <- tibble(Month = seq(min(admin_mb_actual_spent_long$Month), 
                                  today(), 
                                  by = "month"))


# Merge the full month sequence with the actual data and replace NAs with 0
admin_mb_actual_spent_long <- full_months |> 
  left_join(admin_mb_actual_spent_long, by = "Month") |> 
  mutate(Actual_Spent = replace_na(Actual_Spent, 0))  # Fill missing values with 0


# Create the plot with y-axis limits adjusted
admin_mb_spending_plot <- ggplot(admin_mb_actual_spent_long, aes(x = Month, y = `Actual_Spent`, fill = `Cost Center`)) +
  ...
  scale_fill_paletteer_d("tvthemes::Alexandrite", na.translate = FALSE) +  # Corrected function for discrete fill
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1, size = 26, face = "bold"),  # Rotate and bold x-axis labels for readability
    axis.text.y = element_text(size = 26, face = "bold"),  # Bold y-axis text
    ...
    panel.grid.minor = element_blank(),  # Remove minor grid lines
    plot.background = element_rect(fill = "white", color = NA),  # Set plot background to white
    legend.title = element_text(size = 30, face = "bold"),  # Adjust legend title size
    legend.text = element_text(size = 28),  # Adjust legend text size
...
# Display the plot
print(admin_mb_spending_plot)

# saving the plot
ggsave("admin_mb_monthly_spending_deputy_blye.pdf", 
       width = 45, height = 30)
```
