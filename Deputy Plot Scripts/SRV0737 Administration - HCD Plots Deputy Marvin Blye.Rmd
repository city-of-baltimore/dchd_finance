...
admin_nh_negative_spending <- budg_vs_actuals |>
  filter(
...
admin_nh_tot_neg_ccenter <- ggplot(admin_nh_negative_spending, aes(x = reorder(`Cost Center`, `Total Spent`), y = `Total Spent`)) +
...
# display the plot
print(admin_nh_tot_neg_ccenter)

# saving the plot
ggsave("plots/admin_nh_tot_neg_ccenter_deputy_marvin.pdf", 
       width = 30, height = 20)
```
...
```{r spendcat}
# plotting data below for different perspectives
admin_nh_tot_neg_spcat <- ggplot(admin_nh_negative_spending, aes(x = reorder(`Spend Category`, `Total Spent`), y = `Total Spent`)) +
  geom_col(fill = "red", width = 0.5) +  # Adjust bar width for better aesthetics
  theme_minimal() +
  scale_y_reverse(labels = scales::dollar) +  # Reverse y-axis and format with dollar labels
  labs(
...
# display the plot
print(admin_nh_tot_neg_spcat)

# saving the plot
ggsave("plots/admin_nh_tot_neg_spcat_deputy_marvin.pdf", 
       width = 30, height = 20)
```
...
admin_nh_actual_spent <- budg_vs_actuals |>
  filter(
    Service == "SRV0737 Administration - HCD" &
    `Cost Center` %in% c("CCA000310 Housing Budget and Accounting (HCD)",
                         "CCA000311 Housing Communications (HCD)",
...
admin_nh_max_spent_data <- admin_nh_actual_spent |> 
...
admin_nh_july_spending_plot <- ggplot(admin_nh_max_spent_data, aes(x = `Cost Center`, y = `Actuals_2024-07`)) +
  ...
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1, size = 18),  # Rotate x-axis labels for readability
    axis.text.y = element_text(size = 30),  # Adjust y-axis text size
    axis.title.x = element_text(size = 26, face = "bold"),  # Emphasize x-axis title
    axis.title.y = element_text(size = 26, face = "bold"),  # Emphasize y-axis title
    plot.title = element_text(size = 30, face = "bold", hjust = 0.5),  # Center-align and enlarge title
    plot.subtitle = element_text(size = 28, hjust = 0.5, face = "italic"),  # Style subtitle
    plot.caption = element_text(size = 20, face = "italic"),  # Style caption
    panel.grid.major = element_line(color = "gray90"),  # Lighter grid for subtle effect
    panel.grid.minor = element_blank(),  # Remove minor grid lines
    plot.background = element_rect(fill = "white", color = NA),  # Set plot background to white
...
# Display the plot
print(admin_nh_july_spending_plot)

# saving the plot
ggsave("plots/admin_nh_july_spending_deputy_marvin.pdf", 
       width = 45, height = 30)
```
