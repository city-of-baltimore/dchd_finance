...
# saving the plot
ggsave("csp_nh_tot_neg_ccenter_deputy_Hart.pdf",
       width = 30, height = 20)
```
...
# saving the plot
ggsave("csp_nh_tot_neg_spcat_deputy_Hart.pdf",
       width = 30, height = 20)
```
...
csp_nh_actual_spent_long <- csp_nh_actual_spent |> 
  pivot_longer(
...
csp_nh_actual_spent_long <- csp_nh_actual_spent_long |> 
  filter(!is.na(Month))

# Now, create a sequence of months from the minimum to the current month
full_months <- tibble(Month = seq(min(csp_nh_actual_spent_long$Month), 
                                  today(), 
                                  by = "month"))

# Merge the full month sequence with the actual data and replace NAs with 0
csp_nh_actual_spent_long <- full_months |> 
  left_join(csp_nh_actual_spent_long, by = "Month") |> 
  mutate(Actual_Spent = replace_na(Actual_Spent, 0))  # Fill missing values with 0


# Create the plot with y-axis limits adjusted
# Create the plot with y-axis limits adjusted
csp_nh_spending_plot <- ggplot(csp_nh_actual_spent_long, aes(x = Month, y = `Actual_Spent`, fill = `Cost Center`)) +
...
# Display the plot
print(csp_nh_spending_plot)

# saving the plot
ggsave("csp_nh_monthly_spending_deputy_Hart.pdf", width = 30, height = 20)
```
