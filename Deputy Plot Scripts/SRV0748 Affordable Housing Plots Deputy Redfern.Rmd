---
title: "SRV0748 Affordable Housing Plots Deputy Wendi Redfern"
...
*Note: This will make plots for `SRV0593 Community Support Projects` only and will be for Deputy Wendi Redfern*
...
*Service* 
  1.`SRV0748 Affordable Housing`
*Cost Center* 
  1.`CCA000332 Housing Development Finance and Project Management` 
  2.`CCA000333 Affordable Housing Trust Fund Program` 
  3.`CCA000334 Affordable Housing Unallocated Appropriation` 
  4.`CCA001406 Hope Rent Support Progam`
...
```{r totspentnegplot, message=FALSE, warning=FALSE}
# filter the dataset for negative spending values
ah_negative_spending <- budg_vs_actuals |>
  filter(
    Service == "SRV0748 Affordable Housing" &
    `Cost Center` %in% c("CCA000332 Housing Development Finance and Project Management", 
                         "CCA000333 Affordable Housing Trust Fund Program",
                         "CCA000334 Affordable Housing Unallocated Appropriation",
                         "CCA001406 Hope Rent Support Progam") &
    `Total Spent` < 0
  )

# plotting data below for different perspectives
ah_tot_neg_ccenter <- ggplot(ah_negative_spending, aes(x = reorder(`Cost Center`, `Total Spent`), y = `Total Spent`)) +
  ...
  labs(
    ...
    subtitle = "Service: SRV0748 Affordable Housing",
    x = "Cost Center",
    y = "Total Spent (USD)",
...
# display the plot
print(ah_tot_neg_ccenter)

# saving the plot
ggsave("ah_tot_neg_ccenter_deputy_Redfern.pdf", 
       width = 30, height = 20)
```
...
```{r spendcat, message=FALSE, warning=FALSE}
# plotting data below for different perspectives
ah_tot_neg_spcat <- ggplot(ah_negative_spending, aes(x = reorder(`Spend Category`, `Total Spent`), y = `Total Spent`)) +
  geom_col(fill = "red", width = 0.5) +  # Adjust bar width for better aesthetics
  theme_minimal() +  # Minimalistic theme
  scale_y_reverse(labels = scales::dollar) +  # Reverse y-axis and format with dollar labels
  labs(
...
# display the plot
print(ah_tot_neg_spcat)

# saving the plot
ggsave("ah_tot_neg_spcat_deputy_Redfern.pdf", 
       width = 30, height = 20)
```
...
ah_actual_spent <- budg_vs_actuals |>
  filter(
    Service == "SRV0748 Affordable Housing" &
    `Cost Center` %in% c("CCA000332 Housing Development Finance and Project Management", 
                         "CCA000333 Affordable Housing Trust Fund Program",
...
ah_actual_spent_long <- ah_actual_spent |> 
  pivot_longer(
...
ah_actual_spent_long <- ah_actual_spent_long |> 
  filter(!is.na(Month))

# Now, create a sequence of months from the minimum to the current month
full_months <- tibble(Month = seq(min(ah_actual_spent_long$Month), 
                                  today(), 
                                  by = "month"))


# Merge the full month sequence with the actual data and replace NAs with 0
ah_actual_spent_long <- full_months |> 
  left_join(ah_actual_spent_long, by = "Month") |> 
  mutate(Actual_Spent = replace_na(Actual_Spent, 0))  # Fill missing values with 0


# Create the plot with y-axis limits adjusted
ah_actual_spent_plot <- ggplot(ah_actual_spent_long, aes(x = Month, y = `Actual_Spent`, fill = `Cost Center`)) +
...
# saving the plot
ggsave("ah_monthly_spending_deputy_Redfern.pdf",
       width = 45, height = 30)
```
