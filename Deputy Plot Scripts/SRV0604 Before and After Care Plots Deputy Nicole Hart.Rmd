---
title: "SVR0604 Before and After Care Plots Deputy Nicole Hart"
...
*Note: This will make plots for `SRV0604 Before and After Care` only and will be for Deputy Nicole Hart*
...
*Service* 
  1.`SRV0604 Before and After Care`
*Cost Center* 
  1.`CCA000307 Northwood Child Care Center`
  2.`CCA000308 Waverly Child Care Center`
...
bac_nh_negative_spending <- budg_vs_actuals |>
  filter(
    Service == "SRV0604 Before and After Care" &
    `Cost Center` %in% c("CCA000307 Northwood Child Care Center", 
                         "CCA000308 Waverly Child Care Center") &
    `Total Spent` < 0
  )

# plotting data below for different perspectives
bac_nh_tot_neg_ccenter <- ggplot(bac_nh_negative_spending, aes(x = reorder(`Cost Center`, `Total Spent`), y = `Total Spent`)) +
  ...
  labs(
    ...
    subtitle = "Service: SRV0604 Before and After Care",
    x = "Cost Center",
    y = "Total Spent (USD)",
...
# saving the plot
ggsave("bac_nh_tot_neg_ccenter_deputy_Hart.pdf", width = 30, height = 20) # this will save to your local drive
```
...
```{r spendcat}
# plotting data below for different perspectives
bac_nh_tot_neg_spcat <- ggplot(bac_nh_negative_spending, aes(x = reorder(`Spend Category`, `Total Spent`), y = `Total Spent`)) +
  geom_col(fill = "red", width = 0.5) +  # Adjust bar width for better aesthetics
  theme_minimal() +
  scale_y_reverse(labels = scales::dollar) +  # Reverse y-axis and format with dollar labels
  labs(
...
# saving the plot
ggsave("bac_nh_tot_neg_spcat_deputy_Hart.pdf", width = 30, height = 20)
```
...
bac_nh_actual_spent <- budg_vs_actuals |>
  filter(
    Service == "SRV0604 Before and After Care" &
    `Cost Center` %in% c("CCA000307 Northwood Child Care Center", 
                         "CCA000308 Waverly Child Care Center")
...
bac_nh_actual_spent_long <- bac_nh_actual_spent |> 
  pivot_longer(
...
# saving the plot
ggsave("bac_nh_monthly_deputy_Hart.pdf", width = 30, height = 20)
```
