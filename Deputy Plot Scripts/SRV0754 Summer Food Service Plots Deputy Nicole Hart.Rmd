---
title: "SRV0754 Summer Food Service Program Plots Deputy Nicole Hart"
...
*Note: This will make plots for `SRV0754 Summer Food Service Program` only and will be for Deputy Nicole Hart*
...
*Service* 
  1.`SRV0754 Summer Food Service Program`
*Cost Center* 
  1.`CCA000347 Summer Food Service Program`
...
sfsp_nh_negative_spending <- budg_vs_actuals |>
  filter(
    Service == "SRV0754 Summer Food Service Program" &
    `Cost Center` %in% c("CCA000347 Summer Food Service Program") &
    `Total Spent` < 0
  )

# plotting data below for different perspectives
sfsp_nh_tot_neg_ccenter <- ggplot(sfsp_nh_negative_spending, aes(x = reorder(`Cost Center`, `Total Spent`), y = `Total Spent`)) +
  ...
  labs(
    ...
    subtitle = "Service: SRV0754 Summer Food Service Program",
    x = "Cost Center",
    y = "Total Spent (USD)",
...
# display the plot
print(sfsp_nh_tot_neg_ccenter)

# saving the plot
ggsave("sfsp_nh_tot_neg_ccenter_deputy_nicole.pdf", 
       width = 30, height = 20)
```
...
```{r spendcat}
# plotting data below for different perspectives
sfsp_nh_tot_neg_spcat <- ggplot(sfsp_nh_negative_spending, aes(x = reorder(`Spend Category`, `Total Spent`), y = `Total Spent`)) +
  geom_col(fill = "red", width = 0.5) +  # Adjust bar width for better aesthetics
  theme_minimal() +
  scale_y_reverse(labels = scales::dollar) +  # Reverse y-axis and format with dollar labels
  labs(
...
# display the plot
print(sfsp_nh_tot_neg_spcat)

# saving the plot
ggsave("sfsp_nh_tot_neg_spcat_deputy_nicole.pdf", 
       width = 30, height = 20)
```
...
sfsp_nh_actual_spent <- budg_vs_actuals |>
  filter(
    Service == "SRV0754 Summer Food Service Program" &
    `Cost Center` %in% c("CCA000347 Summer Food Service Program")
  )
...
sfsp_nh_actual_spent_long <- sfsp_nh_actual_spent |> 
  pivot_longer(
...
sfsp_nh_actual_spent_long <- sfsp_nh_actual_spent_long |> 
  filter(!is.na(Month))

# Now, create a sequence of months from the minimum to the current month
full_months <- tibble(Month = seq(min(sfsp_nh_actual_spent_long$Month), 
                                  today(), 
                                  by = "month"))


# Merge the full month sequence with the actual data and replace NAs with 0
sfsp_nh_actual_spent_long <- full_months |> 
  left_join(sfsp_nh_actual_spent_long, by = "Month") |> 
  mutate(Actual_Spent = replace_na(Actual_Spent, 0))  # Fill missing values with 0


# Create the plot with y-axis limits adjusted
sfsp_nh_actual_spent_plot <- ggplot(sfsp_nh_actual_spent_long, aes(x = Month, y = `Actual_Spent`, fill = `Cost Center`)) +
...
# Display the plot
print(sfsp_nh_actual_spent_plot)

# saving the plot
ggsave("sfsp_nh_monthly_spending_deputy_Hart.pdf", 
       width = 30, height = 20)
```
