---
title: "SRV0751 Building Code Permitting, Inspections and Compliance Plots Deputy Jason Hessler"
...
*Note: This will make plots for `` only and will be for Deputy Jason Hessler*
...
*Service* 
  1.`SRV0751 Building Code Permitting, Inspections and Compliance`
*Cost Center* 
  1.`CCA000341 Construction Code Enforcement (HCD)`
  2.`CCA000342 Building and Zoning Demolition Inspection (HCD)`
  3.`CCA000343 Building and Zoning Permit Processing (HCD)`
  4.`CCA000344 Building and Zoning Plans Examining (HCD)`
  5.`CCA000345 Building and Zoning Plans Enforcement (HCD)`
...
bzip_jh_negative_spending <- budg_vs_actuals |>
  filter(
    Service == "SRV0751 Building Code Permitting, Inspections and Compliance" &
    `Cost Center` %in% c("CCA000341 Construction Code Enforcement (HCD)",
                         "CCA000342 Building and Zoning Demolition Inspection (HCD)",
                         "CCA000343 Building and Zoning Permit Processing (HCD)",
                         "CCA000344 Building and Zoning Plans Examining (HCD)",
                         "CCA000345 Building and Zoning Plans Enforcement (HCD)") &
    `Total Spent` < 0
  )

# plotting data below for different perspectives
bzip_jh_tot_neg_ccenter <- ggplot(bzip_jh_negative_spending, aes(x = reorder(`Cost Center`, `Total Spent`), y = `Total Spent`)) +
  ...
  labs(
    ...
    subtitle = "Service: SRV0751 Building Code Permitting, Inspections and Compliance",
    x = "Cost Center",
    y = "Total Spent (USD)",
...
# display the plot
print(bzip_jh_tot_neg_ccenter)

# saving the plot
ggsave("bzip_jh_tot_neg_ccenter_deputy_Hessler.pdf",
       width = 30, height = 20)
```
...
# plotting data below for different perspectives
bzip_jh_tot_neg_spcat <- ggplot(bzip_jh_negative_spending, aes(x = reorder(`Spend Category`, `Total Spent`), y = `Total Spent`)) +
  geom_col(fill = "red", width = 0.5) +  # Adjust bar width for better aesthetics
  theme_minimal() +
  scale_y_reverse(labels = scales::dollar) +  # Reverse y-axis and format with dollar labels
  labs(
...
# display the plot
print(bzip_jh_tot_neg_spcat)

# saving the plot
ggsave("bzip_jh_tot_neg_spcat_deputy_Hessler.pdf",
       width = 30, height = 20)
```
...
bzip_jh_actual_spent <- budg_vs_actuals |>
  filter(
    Service == "SRV0751 Building Code Permitting, Inspections and Compliance" &
    `Cost Center` %in% c("CCA000341 Construction Code Enforcement (HCD)",
                         "CCA000342 Building and Zoning Demolition Inspection (HCD)",
...
bzip_jh_actual_spent_long <- bzip_jh_actual_spent |> 
  pivot_longer(
...
bzip_jh_actual_spent_long <- bzip_jh_actual_spent_long |> 
  filter(!is.na(Month))

# Now, create a sequence of months from the minimum to the current month
full_months <- tibble(Month = seq(min(bzip_jh_actual_spent_long$Month), 
                                  today(), 
                                  by = "month"))


# Merge the full month sequence with the actual data and replace NAs with 0
bzip_jh_actual_spent_long <- full_months |> 
  left_join(bzip_jh_actual_spent_long, by = "Month") |> 
  mutate(Actual_Spent = replace_na(Actual_Spent, 0))  # Fill missing values with 0


# Create the plot with y-axis limits adjusted
bzip_jh_actual_spent_plot <- ggplot(bzip_jh_actual_spent_long, aes(x = Month, y = `Actual_Spent`, fill = `Cost Center`)) +
...
# Display the plot
print(bzip_jh_actual_spent_plot)

# saving the plot
ggsave("bzip_jh_monthly_spending_deputy_Hessler.pdf",
       width = 45, height = 30)
```
```
