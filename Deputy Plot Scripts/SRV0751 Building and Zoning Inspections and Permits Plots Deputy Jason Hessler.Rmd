---
title: "SRV0751 Building and Zoning Inspections and Permits Plots Deputy Jason Hessler"
...
*Note: This will make plots for `SRV0751 Building and Zoning Inspections and Permits` only and will be for Deputy Jason Hessler*
...
*Service* 
  1.`SRV0751 Building and Zoning Inspections and Permits`
*Cost Center* 
...
bzip_nh_negative_spending <- budg_vs_actuals |>
  filter(
    Service == "SRV0751 Building and Zoning Inspections and Permits" &
...
bzip_nh_tot_neg_ccenter <- ggplot(bzip_nh_negative_spending, aes(x = reorder(`Cost Center`, `Total Spent`), y = `Total Spent`)) +
  ...
  labs(
    ...
    subtitle = "Service: SRV0751 Building and Zoning Inspections and Permits",
    x = "Cost Center",
    y = "Total Spent (USD)",
...
# display the plot
print(bzip_nh_tot_neg_ccenter)

# saving the plot
ggsave("bzip_nh_tot_neg_ccenter_deputy_jason.pdf",
       width = 30, height = 20)
```
...
```{r spendcat}
# plotting data below for different perspectives
bzip_nh_tot_neg_spcat <- ggplot(bzip_nh_negative_spending, aes(x = reorder(`Spend Category`, `Total Spent`), y = `Total Spent`)) +
  geom_col(fill = "red", width = 0.5) +  # Adjust bar width for better aesthetics
  theme_minimal() +
  scale_y_reverse(labels = scales::dollar) +  # Reverse y-axis and format with dollar labels
  labs(
...
# display the plot
print(bzip_nh_tot_neg_spcat)

# saving the plot
ggsave("bzip_nh_tot_neg_spcat_deputy_jason.pdf",
       width = 30, height = 20)
```
...
bzip_nh_actual_spent <- budg_vs_actuals |>
  filter(
    Service == "SRV0751 Building and Zoning Inspections and Permits" &
    `Cost Center` %in% c("CCA000341 Construction Code Enforcement (HCD)",
                         "CCA000342 Building and Zoning Demolition Inspection (HCD)",
...
bzip_nh_max_spent_data <- bzip_nh_actual_spent |> 
...
bzip_nh_july_spending_plot <- ggplot(bzip_nh_max_spent_data, aes(x = `Cost Center`, y = `Actuals_2024-07`)) +
...
# Display the plot
print(bzip_nh_july_spending_plot)

# saving the plot
ggsave("bzip_nh_july_spending_deputy_jason.pdf", 
       width = 45, height = 30)
```
