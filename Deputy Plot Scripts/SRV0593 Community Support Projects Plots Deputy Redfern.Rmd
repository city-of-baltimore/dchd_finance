---
title: "SRV0593 Community Support Projects Plots Deputy Wendi Redfern"
...
*Cost Center* 
  1.`CCA000248 Community Support Projects Unallocated Appropriation (HCD)`
  2.`CCA000249 Community Catalyst Grants (HCD)`
...
csp_negative_spending <- budg_vs_actuals |>
  filter(
...
csp_tot_neg_ccenter <- ggplot(csp_negative_spending, aes(x = reorder(`Cost Center`, `Total Spent`), y = `Total Spent`)) +
...
# display the plot
print(csp_tot_neg_ccenter)

# saving the plot
ggsave("csp_tot_neg_ccenter_deputy_redfern.pdf", 
       width = 30, height = 20)
```
...
# plotting data below for different perspectives
csp_tot_neg_spcat <- ggplot(csp_negative_spending, aes(x = reorder(`Spend Category`, `Total Spent`), y = `Total Spent`)) +
  geom_col(fill = "red", width = 0.5) +  # Adjust bar width for better aesthetics
  theme_minimal() +  # Minimalistic theme
  scale_y_reverse(labels = scales::dollar) +  # Reverse y-axis and format with dollar labels
  labs(
...
# display the plot
print(csp_tot_neg_spcat)

# saving the plot
ggsave("csp_tot_neg_spcat_deputy_redfern.pdf", 
       width = 30, height = 20)
```
...
csp_actual_spent <- budg_vs_actuals |>
  filter(
    Service == "SRV0593 Community Support Projects" &
    `Cost Center` %in% c("CCA000259 Community Support Projects Administration", 
                         "CCA000249 Community Catalyst Grants (HCD)")
...
csp_actual_spent_long <- csp_actual_spent |> 
  pivot_longer(
...
csp_actual_spent_long <- csp_actual_spent_long |> 
  filter(!is.na(Month))

# Now, create a sequence of months from the minimum to the current month
full_months <- tibble(Month = seq(min(csp_actual_spent_long$Month), 
                                  today(), 
                                  by = "month"))

# Merge the full month sequence with the actual data and replace NAs with 0
csp_actual_spent_long <- full_months |> 
  left_join(csp_actual_spent_long, by = "Month") |> 
  mutate(Actual_Spent = replace_na(Actual_Spent, 0))  # Fill missing values with 0


# Create the plot with y-axis limits adjusted
# Create the plot with y-axis limits adjusted
csp_spending_plot <- ggplot(csp_actual_spent_long, aes(x = Month, y = `Actual_Spent`, fill = `Cost Center`)) +
...
# Display the plot
print(csp_spending_plot)

# saving the plot
ggsave("csp_monthly_spending_deputy_redfern.pdf", width = 30, height = 20)
```
