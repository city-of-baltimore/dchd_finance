---
title: "SRV0752 Community Outreach Services Plots Deputy Nicole Hart"
...
*Note: This will make plots for `SRV0752 Community Outreach Services` only and will be for Deputy Nicole Hart*
...
*Service* 
  1.`SRV0752 Community Outreach Services`
*Cost Center* 
  1.`CCA000346 Housing Ombudsmans Office (HCD)`
  2.`CCA001338 Emergency Services (HCD)`
  3.`CCA001339 Human Services (HCD)`
...
cos_nh_negative_spending <- budg_vs_actuals |>
  filter(
    Service == "SRV0752 Community Outreach Services" &
    `Cost Center` %in% c("CCA000346 Housing Ombudsmans Office (HCD)", 
                         "CCA001338 Emergency Services (HCD)",
                         "CCA001339 Human Services (HCD)") &
    `Total Spent` < 0
  )

# plotting data below for different perspectives
cos_nh_tot_neg_ccenter <- ggplot(cos_nh_negative_spending, aes(x = reorder(`Cost Center`, `Total Spent`), y = `Total Spent`)) +
  ...
  labs(
    ...
    subtitle = "Service: SRV0752 Community Outreach Services",
    x = "Cost Center",
    y = "Total Spent (USD)",
...
# display the plot
print(cos_nh_tot_neg_ccenter)

# saving the plot
ggsave("cos_nh_tot_neg_ccenter_deputy_Hart.pdf",
       width = 30, height = 20)
```
...
```{r spendcat}
# plotting data below for different perspectives
cos_nh_tot_neg_spcat <- ggplot(cos_nh_negative_spending, aes(x = reorder(`Spend Category`, `Total Spent`), y = `Total Spent`)) +
  geom_col(fill = "red", width = 0.5) +  # Adjust bar width for better aesthetics
  theme_minimal() +
  scale_y_reverse(labels = scales::dollar) +  # Reverse y-axis and format with dollar labels
  labs(
...
# display the plot
print(cos_nh_tot_neg_spcat)

# saving the plot
ggsave("cos_nh_tot_neg_spcat_deputy_Hart.pdf", 
       width = 30, height = 20)
```
...
cos_nh_actual_spent <- budg_vs_actuals |>
  filter(
    Service == "SRV0752 Community Outreach Services" &
    `Cost Center` %in% c("CCA000346 Housing Ombudsmans Office (HCD)", 
                         "CCA001338 Emergency Services (HCD)",
...
cos_nh_actual_spent_long <- cos_nh_actual_spent |> 
  pivot_longer(
...
cos_nh_actual_spent_long <- cos_nh_actual_spent_long |> 
  filter(!is.na(Month))

# Now, create a sequence of months from the minimum to the current month
full_months <- tibble(Month = seq(min(cos_nh_actual_spent_long$Month), 
                                  today(), 
                                  by = "month"))


# Merge the full month sequence with the actual data and replace NAs with 0
cos_nh_actual_spent_long <- full_months |> 
  left_join(cos_nh_actual_spent_long, by = "Month") |> 
  mutate(Actual_Spent = replace_na(Actual_Spent, 0))  # Fill missing values with 0


# Create the plot with y-axis limits adjusted
cos_nh_actual_spent_plot <- ggplot(cos_nh_actual_spent_long, aes(x = Month, y = `Actual_Spent`, fill = `Cost Center`)) +
...
# Display the plot
print(cos_nh_actual_spent_plot)

# saving the plot
ggsave("cos_nh_monthly_spending_deputy_Hart.pdf", 
       width = 30, height = 20)
```
