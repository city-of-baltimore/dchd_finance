---
title: "SRV0742 Promote Homeownership"
...
*Note: This will make plots for `SRV0742 Promote Homeownership` only and will be for Deputy Nicole Hart*
...
*Service* 
  1.`SRV0742 Promote Homeownership`
*Cost Center* 
  1.`CCA000321 Homeownership`
  2.`CCA001302 Casino Support Homeownership Incentives`
  3.`CCA001303 Tax Sale Prevention (DHCD)`
  4.`CCA001409 Casino Support Community Development`
...
ph_nh_negative_spending <- budg_vs_actuals |>
  filter(
    Service == "SRV0742 Promote Homeownership" &
    `Cost Center` %in% c("CCA000321 Homeownership", 
                         "CCA001302 Casino Support Homeownership Incentives",
                         "CCA001303 Tax Sale Prevention (DHCD)",
                         "CCA001409 Casino Support Community Development") &
    `Total Spent` < 0
  )

# plotting data below for different perspectives
ph_nh_tot_neg_ccenter <- ggplot(ph_nh_negative_spending, aes(x = reorder(`Cost Center`, `Total Spent`), y = `Total Spent`)) +
  ...
  labs(
    ...
    subtitle = "Service: SRV0742 Promote Homeownership",
    x = "Cost Center",
    y = "Total Spent (USD)",
...
# display the plot
print(ph_nh_tot_neg_ccenter)

# saving the plot
ggsave("ph_nh_tot_neg_ccenter_deputy_Hart.pdf", 
       width = 30, height = 20)
```
...
```{r spendcat}
# plotting data below for different perspectives
ph_nh_tot_neg_spcat <- ggplot(ph_nh_negative_spending, aes(x = reorder(`Spend Category`, `Total Spent`), y = `Total Spent`)) +
  geom_col(fill = "red", width = 0.5) +  # Adjust bar width for better aesthetics
  theme_minimal() +
  scale_y_reverse(labels = scales::dollar) +  # Reverse y-axis and format with dollar labels
  labs(
...
# display the plot
print(ph_nh_tot_neg_spcat)

# saving the plot
ggsave("ph_nh_tot_neg_spcat_deputy_Hart.pdf", 
       width = 30, height = 20)
```
...
ph_nh_actual_spent <- budg_vs_actuals |>
  filter(
    Service == "SRV0742 Promote Homeownership" &
    `Cost Center` %in% c("CCA000321 Homeownership", 
                         "CCA001302 Casino Support Homeownership Incentives",
...
ph_nh_actual_spent_long <- ph_nh_actual_spent |> 
  pivot_longer(
...
ph_nh_actual_spent_long <- ph_nh_actual_spent_long |> 
  filter(!is.na(Month))

# Now, create a sequence of months from the minimum to the current month
full_months <- tibble(Month = seq(min(ph_nh_actual_spent_long$Month), 
                                  today(), 
                                  by = "month"))


# Merge the full month sequence with the actual data and replace NAs with 0
ph_nh_actual_spent_long <- full_months |> 
  left_join(ph_nh_actual_spent_long, by = "Month") |> 
  mutate(Actual_Spent = replace_na(Actual_Spent, 0))  # Fill missing values with 0


# Create the plot with y-axis limits adjusted
ph_nh_actual_spent_plot <- ggplot(ph_nh_actual_spent_long, aes(x = Month, y = `Actual_Spent`, fill = `Cost Center`)) +
  ...
  theme(
    ...
    panel.grid.minor = element_blank(),  # Remove minor grid lines
    plot.background = element_rect(fill = "white", color = NA),  # Set plot background to white
    legend.title = element_text(size = 20, face = "bold"),  # Adjust legend title size
    legend.text = element_text(size = 18),  # Adjust legend text size
    legend.key.size = unit(3, "lines")  # Increase size of the legend key
  )

# Display the plot
print(ph_nh_actual_spent_plot)

# saving the plot
ggsave("ph_nh_monthly_spending_deputy_Hart.pdf", 
       width = 30, height = 20)
```
