---
title: "SRV0747 Register and License Properties and Contractors Plots Deputy Jason Hessler"
...
*Note: This will make plots for `SRV0747 Register and License Properties and Contractors` only and will be for Deputy Jason Hessler*
...
*Service* 
  1.`SRV0747 Register and License Properties and Contractors`
*Cost Center* 
  1.`CCA000330 Contractor Licensing and Registration (HCD)`
  2.`CCA000331 Property Registration and Multi-Family Dwelling Licensing (HCD)`
...
rlpc_jh_nh_negative_spending <- budg_vs_actuals |>
  filter(
    Service == "SRV0747 Register and License Properties and Contractors" &
    `Cost Center` %in% c("CCA000330 Contractor Licensing and Registration (HCD)",
                         "CCA000331 Property Registration and Multi-Family Dwelling Licensing (HCD)") &
    `Total Spent` < 0
  )

# plotting data below for different perspectives
rlpc_jh_nh_tot_neg_ccenter <- ggplot(rlpc_jh_nh_negative_spending, aes(x = reorder(`Cost Center`, `Total Spent`), y = `Total Spent`, fill = `Cost Center`)) +
  ...
  scale_y_reverse(labels = scales::dollar) +  # Reverse y-axis and format with dollar labels
    ...
    subtitle = "Service: SRV0747 Register and License Properties and Contractors",
    x = "Cost Center",
    y = "Total Spent (USD)",
...
# saving the plot
ggsave("rlpc_jh_nh_tot_neg_ccenter_deputy_jason.pdf",
       width = 30, height = 20)
```
...
# plotting data below for different perspectives
rlpc_jh_nh_tot_neg_spcat <- ggplot(rlpc_jh_nh_negative_spending, aes(x = reorder(`Spend Category`, `Total Spent`), y = `Total Spent`)) +
  geom_col(fill = "red", width = 0.5) +  # Adjust bar width for better aesthetics
  theme_minimal() +
  scale_y_reverse(labels = scales::dollar) +  # Reverse y-axis and format with dollar labels
  labs(
...
# saving the plot
ggsave("rlpc_jh_nh_tot_neg_spcat_deputy_jason.pdf", 
       width = 30, height = 20)
```
...
rlpc_jh_nh_actual_spent <- budg_vs_actuals |>
  filter(
    Service == "SRV0747 Register and License Properties and Contractors" &
    `Cost Center` %in% c("CCA000330 Contractor Licensing and Registration (HCD)",
                         "CCA000331 Property Registration and Multi-Family Dwelling Licensing (HCD)")
...
rlpc_jh_nh_actual_spent_long <- rlpc_jh_nh_actual_spent |> 
  pivot_longer(
...
rlpc_jh_nh_actual_spent_long <- rlpc_jh_nh_actual_spent_long |> 
  filter(!is.na(Month))

# Now, create a sequence of months from the minimum to the current month
full_months <- tibble(Month = seq(min(rlpc_jh_nh_actual_spent_long$Month), 
                                  today(), 
                                  by = "month"))


# Merge the full month sequence with the actual data and replace NAs with 0
rlpc_jh_nh_actual_spent_long <- full_months |> 
  left_join(rlpc_jh_nh_actual_spent_long, by = "Month") |> 
  mutate(Actual_Spent = replace_na(Actual_Spent, 0))  # Fill missing values with 0


# Create the plot with y-axis limits adjusted
rlpc_jh_nh_actual_spent_plot <- ggplot(rlpc_jh_nh_actual_spent_long, aes(x = Month, y = `Actual_Spent`, fill = `Cost Center`)) +
...
# saving the plot
ggsave("rlpc_jh_monthly_spending_deputy_jason.pdf",
       width = 45, height = 30)
```
