...
# saving the plot
ggsave("pda_tot_neg_ccenter_deputy_Redfern.pdf",
       width = 30, height = 20)
```
...
# saving the plot
ggsave("pda_tot_neg_spcat_deputy_Redfern.pdf", 
       width = 30, height = 20)
```
...
pda_actual_spent_long <- pda_actual_spent |> 
  pivot_longer(
...
pda_actual_spent_long <- pda_actual_spent_long |> 
  filter(!is.na(Month))

# Now, create a sequence of months from the minimum to the current month
full_months <- tibble(Month = seq(min(pda_actual_spent_long$Month), 
                                  today(), 
                                  by = "month"))


# Merge the full month sequence with the actual data and replace NAs with 0
pda_actual_spent_long <- full_months |> 
  left_join(pda_actual_spent_long, by = "Month") |> 
  mutate(Actual_Spent = replace_na(Actual_Spent, 0))  # Fill missing values with 0


# Create the plot with y-axis limits adjusted
pda_actual_spent_plot <- ggplot(pda_actual_spent_long, aes(x = Month, y = `Actual_Spent`, fill = `Cost Center`)) +
...
# saving the plot
ggsave("pda_monthly_spending_deputy_Redfern.pdf",
       width = 45, height = 30)
```
