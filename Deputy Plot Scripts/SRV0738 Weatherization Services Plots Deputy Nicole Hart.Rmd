---
title: "SRV0738 Weatherization Services Plots Deputy Nicole Hart"
...
*Note: This will make plots for `SRV0738 Weatherization Services` only and will be for Deputy Nicole Hart*
...
*Service* 
  1.`SRV0738 Weatherization Services`
*Cost Center* 
  1.`CCA000317 Weatherization Program Delivery`
  2.`CCA000318 Weatherization Audit`
  3.`CCA000319 Weatherization Intake Assessment`
...
ws_nh_negative_spending <- budg_vs_actuals |>
  filter(
    Service == "SRV0738 Weatherization Services" &
    `Cost Center` %in% c("CCA000317 Weatherization Program Delivery", 
                         "CCA000318 Weatherization Audit",
                         "CCA000319 Weatherization Intake Assessment") &
    `Total Spent` < 0
  )

# plotting data below for different perspectives
ws_nh_tot_neg_ccenter <- ggplot(ws_nh_negative_spending, aes(x = reorder(`Cost Center`, `Total Spent`), y = `Total Spent`)) +
  ...
  labs(
    ...
    subtitle = "Service: SRV0738 Weatherization Services",
    x = "Cost Center",
    y = "Total Spent (USD)",
...
# display the plot
print(ws_nh_tot_neg_ccenter)

# saving the plot
ggsave("ws_nh_tot_neg_ccenter_deputy_Hart.pdf", 
       width = 30, height = 20)
```
...
```{r spendcat}
# plotting data below for different perspectives
ws_nh_tot_neg_spcat <- ggplot(ws_nh_negative_spending, aes(x = reorder(`Spend Category`, `Total Spent`), y = `Total Spent`)) +
  geom_col(fill = "red", width = 0.5) +  # Adjust bar width for better aesthetics
  theme_minimal() +
  scale_y_reverse(labels = scales::dollar) +  # Reverse y-axis and format with dollar labels
  labs(
...
# display the plot
print(ws_nh_tot_neg_spcat)

# saving the plot
ggsave("ws_nh_tot_neg_spcat_deputy_Hart.pdf",
       width = 30, height = 20)
```
...
ws_nh_actual_spent <- budg_vs_actuals |>
  filter(
    Service == "SRV0738 Weatherization Services" &
    `Cost Center` %in% c("CCA000317 Weatherization Program Delivery", 
                         "CCA000318 Weatherization Audit")
...
ws_nh_actual_spent_long <- ws_nh_actual_spent |> 
  pivot_longer(
...
ws_nh_actual_spent_plot <- ggplot(ws_nh_actual_spent_long, aes(x = Month, y = `Actual_Spent`, fill = `Cost Center`)) +
  ...
  theme(
    axis.text.x = element_text(angle = 30, hjust = 1, size = 22, face = "bold"),  # Rotate and bold x-axis labels for readability
    axis.text.y = element_text(size = 22, face = "bold"),  # Bold y-axis text
    ...
    panel.grid.minor = element_blank(),  # Remove minor grid lines
    plot.background = element_rect(fill = "white", color = NA),  # Set plot background to white
    legend.title = element_text(size = 28, face = "bold"),  # Adjust legend title size
    legend.text = element_text(size = 24),  # Adjust legend text size
    legend.key.size = unit(5, "lines"), # Increase size of the legend key
    legend.spacing.y = unit(5, "cm")  # Add vertical spacing between legend items
  )

# Display the plot
print(ws_nh_actual_spent_plot)

# saving the plot
ggsave("ws_nh_monthly_spending_deputy_Hart.pdf", 
       width = 45, height = 30)
```
