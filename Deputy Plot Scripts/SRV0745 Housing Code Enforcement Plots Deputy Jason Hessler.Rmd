...
hce_jh_actual_spent <- budg_vs_actuals |>
  filter(
    Service == "SRV0745 Housing Code Enforcement" &
    `Cost Center` %in% c("CCA000322 Housing Code Enforcement Administration",
                         "CCA000323 Housing Code Enforcement Legal",
...
hce_jh_actual_spent_long <- hce_jh_actual_spent |> 
  pivot_longer(
...
hce_jh_actual_spent_long <- hce_jh_actual_spent_long |> 
  filter(!is.na(Month))

# Now, create a sequence of months from the minimum to the current month
full_months <- tibble(Month = seq(min(hce_jh_actual_spent_long$Month), 
                                  today(), 
                                  by = "month"))


# Merge the full month sequence with the actual data and replace NAs with 0
hce_jh_actual_spent_long <- full_months |> 
  left_join(hce_jh_actual_spent_long, by = "Month") |> 
  mutate(Actual_Spent = replace_na(Actual_Spent, 0))  # Fill missing values with 0


# Create the plot with y-axis limits adjusted
hce_jh_actual_spent_plot <- ggplot(hce_jh_actual_spent_long, aes(x = Month, y = `Actual_Spent`, fill = `Cost Center`)) +
...
scale_fill_paletteer_d("LaCroixColoR::Apricot", na.translate = FALSE) +
  theme(
...
# Display the plot
print(hce_jh_actual_spent_plot)

# saving the plot
ggsave("hce_jh_monthly_spending_deputy_jason.pdf",
       width = 45, height = 30)
```
