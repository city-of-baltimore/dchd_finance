...
gl_file_name <- "GL-Operating Actual Expenses YTD 2024-12-19 13_03 EST.xlsx" # date will need to be changed

budg_vs_act_file_name <- "Budget_vs_Actuals_-_BBMR YTD 2024-12-19.xlsx" # date will need to be changed
...
# add more months here if needed to complete the analysis
expense_report_tbl <- budg_vs_actuals |> 
  reframe(
    ...
    `Actuals_2024-07` = ifelse(is.na(`Actuals_2024-07`), "", `Actuals_2024-07`), # change the year as needed
...
# create the GL YTD EXP column which sums all of the months together
expense_report_tbl <- expense_report_tbl |> 
  mutate(`GL YTD EXP` = rowSums(across(July:December), na.rm = TRUE))
```

^^^**DOUBLE CHECK THIS BECAUSE THE NUMBERS MATCH THE TOTAL BUD WHICH THEY SHOULD NOT?**
...
tblwhodis <- summer_expense_rep |> 
  mutate(
    ...
    `Remaining Balance` = ifelse(is.na(`GL YTD EXP`), NA, `Total Budget` - `GL YTD EXP`)
  )
