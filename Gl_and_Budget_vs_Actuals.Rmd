...
## Project Overview
This markdown file will be used by the DHCD Finance department. The excel files that are read in come from Workday and are downloaded for analysis. The files pertain to BBMR Baltimore City expenditures and GL data. The data is read into R then an analysis is done so DHCD Finance team can complete their work. There are plots/figures that the team can use for reporting to customers and other agency mem bers that they interact with.  
...
Examples below to mark the benchmarks for the datasets.
