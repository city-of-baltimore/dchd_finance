...
gl_file_name <- basename(get_latest_file("GL-Operating Actual Expenses YTD .*\\.xlsx$"))
budg_vs_act_file_name <- basename(get_latest_file("Budget_vs_Actuals_-_BBMR YTD .*\\.xlsx$"))

# read data from files
all_gl_data <- readxl::read_xlsx(here("Data", gl_file_name), 
                             skip = 13,
...
Calculating the gl_ytd_exp column. I will also rename the `Total Actual` column to the `Total Expenditure` column. Then join the gl table to the all expense report table. 
...
all_tblwhodis <- all_expense_report_tbl |> 
  mutate(
...
all_tblwhodis$Notes <- NA_character_  
```
...
```{r wbs}
# keeping only the rows we want and make sure to move the total expenditures to after the GL_YTD_EXP column
all_expense_report_tbl <- all_expense_report_tbl |> 
  group_by(Fund, `Cost Center`) |> 
  mutate(row_num = row_number()) |> 
  ungroup() |> 
  mutate(Fund = na_if(Fund, ""), `Cost Center` = na_if(`Cost Center`, "")) |> 
  fill(Fund, `Cost Center`, .direction = "down") |> 
  select(-row_num)



# move the Total EXP column to after the GL column
all_expense_report_tbl <- all_expense_report_tbl |> 
  relocate(`BBMR Total Expenditures`, .after = `GL YTD EXP`)


# only keep the columns that are needed for this table
all_analysis_tbl <- all_tblwhodis |> 
...
writeData(wb1, "All Service Centers Report", title_header, startRow = 1, startCol = 1)
...
mergeCells(wb1, "All Service Centers Report", cols = 1:ncol(all_expense_report_tbl), rows = 1)
addStyle(wb1, "All Service Centers Report", header_style, rows = 1, cols = 1:ncol(all_expense_report_tbl), gridExpand = TRUE)

# --- Write the Data (Row 2) with Column Names ---

writeData(wb1, "All Service Centers Report", all_expense_report_tbl, startRow = 2, startCol = 1, colNames = TRUE)

# --- Align and Style Column Headers ---

# Center alignment for column headers
center_style <- createStyle(halign = "center", valign = "center")
addStyle(wb1, "All Service Centers Report", center_style, rows = 2, cols = 1:ncol(all_expense_report_tbl), gridExpand = TRUE)
...
addStyle(wb1, "All Service Centers Report", column_names_style, rows = 2, cols = 1:ncol(all_expense_report_tbl), gridExpand = TRUE)
...
writeData(wb2, "all_analysis_tbl", all_analysis_tbl, startRow = 2, startCol = 1, colNames = TRUE)
...
# merge the cells
mergeCells(wb2, "all_analysis_tbl", cols = 1:ncol(all_analysis_tbl), rows = 1)
addStyle(wb2, "all_analysis_tbl", header_style, rows = 1, cols = 1:ncol(all_analysis_tbl), gridExpand = TRUE)

# --- align for column headers ---

# center alignment for column headers
center_style <- createStyle(halign = "center", valign = "center")
addStyle(wb2, "all_analysis_tbl", center_style, rows = 2, cols = 1:ncol(all_analysis_tbl), gridExpand = TRUE)
...
addStyle(wb2, "all_analysis_tbl", column_names_style, rows = 2, cols = 1:ncol(all_analysis_tbl), gridExpand = TRUE)
