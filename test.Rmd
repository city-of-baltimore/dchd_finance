...
# Define month names dynamically
month_names <- c("January", "February", "March", "April", "May", "June", 
                 "July", "August", "September", "October", "November", "December")

# Ensure all actual columns exist before summarizing
...
expense_report_tbl <- budg_vs_actuals |> 
  group_by(Fund, `Spend Category`, `Cost Center`, Service) |>  # Added Service to group_by
  summarise(
    `Revised Budget` = sum(`Revised Budget`, na.rm = TRUE),
    across(starts_with("Actuals_"), \(x) sum(x, na.rm = TRUE)),  # Sum Actuals columns
    .groups = "drop"
  )

# Convert actuals columns to numeric safely
expense_report_tbl <- expense_report_tbl |> 
  mutate(across(starts_with("Actuals_"), as.numeric))  # Ensure actuals are numeric

# Rename columns to the "Month Year" format (e.g., "June 2024")
expense_report_tbl <- expense_report_tbl |> 
  rename_with(
    ~ {
      # Extract year and month part from the column name
      date_part <- sub("^Actuals_(\\d{4})-(\\d{2})$", "\\1-\\2", .)
      year <- sub("^(\\d{4})-(\\d{2})$", "\\1", date_part)
      month <- as.numeric(sub("^(\\d{4})-(\\d{2})$", "\\2", date_part))
      month_name <- month_names[month]
      
      # Return the formatted "Month Year"
...
all_expense_rep <- expense_report_tbl |> 
  arrange(`Service`, `Spend Category`) |> 
  select(
    `Service`, 
    Fund, 
    `Cost Center`, 
